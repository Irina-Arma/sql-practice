/*
Работа с таблицей trip: создание таблицы, добавление данных и выборки командировок.
Используются фильтрация (WHERE, LIKE, IN, NOT IN), сортировка (ORDER BY),
группировка (GROUP BY, HAVING), агрегатные функции (COUNT, SUM),
функции работы с датами (DATEDIFF, MONTH, MONTHNAME, YEAR),
подзапросы, оператор LIMIT, вычисляемые поля и псевдонимы колонок (AS).
*/
